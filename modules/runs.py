...
def initialize_runs_file():
    ...
    if not os.path.exists(RUNS_FILE):
        with open(RUNS_FILE, 'w', newline='') as f:
            writer = csv.writer(f)
            writer.writerow(["Date", "Player Name", "Playtime (Minutes)", "Playtime (Seconds)", "Result"])
...
def log_run(player_name, start_time, end_condition):
    ...
    Args:
        ...
        end_condition: How the run ended (e.g., "Game Completed", "Starved to Death", "Riddle Failure", "Quit")
    """
